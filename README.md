...
Agent 运行记录已经接入侧栏“Agent运行记录”：剧本解析、调度草案、自动排班、对词和剧本问答都会保存结构化摘要、运行模式、步骤 trace、耗时和降级原因。记录按用户隔离，只保存可审计的摘要与引用信息，便于复盘一次 Agent 为什么做出当前结果。
...
- 待继续：资源变更审计，以及高频失败步骤的可观测指标和重试策略
