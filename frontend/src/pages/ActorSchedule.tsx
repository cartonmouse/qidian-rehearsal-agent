    ...
    <div className="space-y-5">
      ...
      {schedule.resource_context && (
        <div className="rounded-xl border border-border bg-background/35 px-3 py-3">
          ...
          {schedule.resource_context.warnings.map((warning) => (
            <div key={warning} className="mt-2 rounded-lg bg-red/8 px-2.5 py-1.5 text-[11px] leading-5 text-red">{warning}</div>
          ))}
        </div>
      )}
