...
- 输出：场次、角色、台词、道具、来源行号和 Agent trace。
- 解析策略：默认 `auto`；无模型配置时使用本地规则，有配置时尝试 LLM 结构化抽取，失败后按场次回退规则。
- 证据约束：台词和来源行号由服务端保留；即使 LLM 返回了有效行号但改写了台词，服务端也以原文为准，人工审核只能修改场次标题、角色和道具元数据。
...
排班 Agent 为一个任务寻找所需全部演员的最早共同空闲区间，并维护演员已占用时间：

- 找到交集：标记 `scheduled` 并记录日期、开始和结束时间；
- 缺少演员档期：标记 `unassigned`，说明缺少哪些演员的时间；
- 有档期但没有共同区间：标记 `unassigned`，原因是演员之间没有共同的空闲时间段。

同一并行组中的任务可以复用同一时间窗口，但单个演员不能被两个任务同时占用。
...
调度草案还会在生成时读取当前用户的配乐时间轴、预算条目、发票元数据和服装库存，写入 `resource_context` 快照。快照包含配乐提示点、预算项目、发票记录、服装库存、预计/实际金额、发票总额、已核验金额和库存风险；当资源非空时，工具调用链会增加 `inspect_rehearsal_resources`，运行记录 trace 会说明读取了哪些资源。实际预算超过预计金额、发票未关联预算项目、仍待核验或服装状态为维修中/缺失/数量为 0 时，只触发“请人工确认”的 warning，不会自动改写预算、确认付款或把不可用服装当成可用。资源在草案生成后发生变化时，需要重新生成调度以获得新的快照。
...
响应同时给出每一项的需求数量、可用数量、匹配原因、汇总和 warnings。服装库存先作为可维护的知识资产接入，当前版本不从自然语言剧本中猜测服装需求。
...
- Resource-aware scheduling：并行组和自动排班都基于演员、道具等资源集合，而不是只按场次顺序。
