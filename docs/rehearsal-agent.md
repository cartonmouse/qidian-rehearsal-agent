...
排练调度请求可以配置 `costume_changeover_minutes`（默认 10，范围 0–120），该值会写入 `resource_context` 快照和调度工具 trace。同一演员的前后场次如果服装集合发生变化，Agent 会把上一场结束后的缓冲区间加入演员 busy 约束；服装集合相同则不额外占用时间。若档期只够排练时长、不够覆盖换装缓冲，任务会标记为 `unassigned`，并说明“换装缓冲没有可用空间”，而不是笼统返回无共同档期。
...
排班 Agent 为一个任务寻找所需全部演员的最早共同空闲区间，并同时维护演员、道具和服装的已占用时间；服装占用按 `resource_context.costume_capacities` 做容量检查，服装变化产生的换装缓冲按 `resource_context.costume_changeover_minutes` 做演员时间约束：

- 找到交集：标记 `scheduled` 并记录日期、开始和结束时间；
- 缺少演员档期：标记 `unassigned`，说明缺少哪些演员的时间；
- 有档期但没有共同区间：标记 `unassigned`，原因是演员之间没有共同的空闲时间段、换装缓冲没有可用空间，或排练资源没有可用并行容量。
...
- Resource explainability：资源检查保留需求数量、可用数量和“缺失/维修中”的具体原因；服装借还通过显式状态动作记录持有人和预计归还，调度快照扣除已借出容量；排练室预约用后端区间冲突校验保证一致性。
